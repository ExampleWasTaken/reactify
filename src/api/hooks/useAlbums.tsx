...
export const useAlbums = () => {
  ...
  const getAlbum = async (id: string, market?: Market): Promise<Album> => {
    const url = await spotify.buildUrl(`/albums/${id}`, new SearchParams({ market: market }));
  ...
  const getAlbums = async (ids: string[], market?: Market): Promise<Album[]> => {
    ...
    const url = await spotify.buildUrl('/albums', new SearchParams({ ids: ids.join(','), market: market }));
  ...
  const getAlbumTracks = async (id: string, market?: Market, limit?: string, offset?: string) => {
    const url = await spotify.buildUrl(`/albums/${id}/tracks`, new SearchParams({ market, limit, offset }));
  ...
  const getUsersSavedAlbums = async (limit?: string, offset?: string, market?: Market) => {
    const url = await spotify.buildUrl('/me/albums', new SearchParams({ limit, offset, market }));
  ...
  const saveAlbumsForCurrentUser = async (ids: string[]) => {
    ...
    const url = await spotify.buildUrl('/me/albums', new SearchParams({ ids: ids.join(',') }));
  ...
  const getNewReleases = async (country?: CountryCodeA2, limit?: string, offset?: string) => {
    const url = await spotify.buildUrl('/browse/new-releases', new SearchParams({ country, limit, offset }));
  ...
  return {
    ...
    removeAlbumsForCurrentUser: deleteAlbumsForCurrentUser,
