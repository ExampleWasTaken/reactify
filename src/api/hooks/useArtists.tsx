...
export const useArtists = () => {
  ...
  const getArtist = async (id: string) => {
    const url = await spotify.buildUrl(`/artists/${id}`);
  ...
  const getArtists = async (ids: string[]) => {
    const url = await spotify.buildUrl('/artists', new SearchParams({ ids: ids.join(',') }));
  ...
  const getTopTracks = async (id: string, market: Market) => {
    const url = await spotify.buildUrl(`/artists/${id}/top-tracks`, new SearchParams({ market }));
  ...
  const getRelatedArtists = async (id: string) => {
    const url = await spotify.buildUrl(`/artists/${id}/related-artists`);
